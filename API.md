...
>
> **实测局限**：赠送模板不支持验证码占位替换——`SendSmsVerifyCode` 的 `templateParam`
> 仅做短信文本替换，无论传 `${code}` 还是字面值，阿里云生成的真实验证码都不会被
> 填入短信（`${code}` 还会被赠送模板的内容校验拒绝）。短信会显示字面值，而
> `CheckSmsVerifyCode` 校验的是内部生成的另一个码，用户永远无法通过。
> **生产可用要求**：在「短信服务」控制台申请内容含 `${code}` 的模板（如
> `验证码${code}，${min}分钟内有效`），将 `SMS_TEMPLATE_CODE_*` 指向该模板，
> 并把 sender 的 `templateParam` 改为 `{"code":"${code}","min":"5"}`——
> 这是官方「短信认证方案」标准做法，`${code}` 会被替换为生成的验证码。
